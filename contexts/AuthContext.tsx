  ...
  useEffect(() => {
    ...
    const initSession = async () => {
      try {
        ...
        if (error) {
          ...
          logger.info('AuthContext: Sessão restaurada', { userId: currentSession.user.id });
        }
      } catch (error) {
        logger.error('AuthContext: Erro na inicialização', error);
      } finally {
        setLoading(false);
      }
    };
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, newSession) => {
      ...
      if (event === 'SIGNED_IN') {
        ...
        logger.debug('AuthContext: Token atualizado');
      }
    });
