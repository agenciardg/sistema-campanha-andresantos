...
DROP POLICY IF EXISTS "Permitir leitura pública de configurações" ON pltdataandrebueno_configuracoes;

CREATE POLICY "Permitir leitura pública de configurações"
ON pltdataandrebueno_configuracoes
FOR SELECT
TO anon, authenticated
USING (true);

-- Garantir que superadmin possa fazer tudo
DROP POLICY IF EXISTS "Superadmin full access on configs" ON pltdataandrebueno_configuracoes;
